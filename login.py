...
class Ui_Login(object):
    ...
    # Fungsi yang dipanggil saat tombol login di klik
    def login_clicked(self):
        email = self.lineEdit.text()
        password = self.lineEdit_2.text()
        user_data = self.read_user_data('users.csv')  # Memanggil fungsi untuk membaca data pengguna dari file users.csv
        if self.validate_login(email, password, user_data):
    ...
    def validate_login(self, email, password, user_data):
